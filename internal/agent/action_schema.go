...
func specificToolInputSchema(toolName string) json.RawMessage {
	...
	case "flow.task.list":
		return json.RawMessage(`{"type":"object","properties":{"query":{"type":"string"},"targetPersonHint":{"type":"string","description":"Leave EMPTY to list everyone's tasks. Set to one person's name — including the requester's own name for their own tasks — to list only that person."},"weekCode":{"type":"string"},"status":{"type":"string"},"limit":{"type":"number"}}}`)
