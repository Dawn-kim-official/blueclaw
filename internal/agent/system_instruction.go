...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " If you know a needed hidden tool or skill by name but it is missing from the current action schema, name it in the requestTools or requestSkills field of your action to expose it for the next step; if nothing useful is callable yet, call skill.search or tool.describe and attach requestTools on that same action. Use skill.search to find skill names. Do not use the separate tool.request action. Never run a Blueclaw tool name as a shell command in terminal.run."
	...
	instruction += " Before you tell the user you lack the capability, permission, access, or data — or before you ask them to provide a system, account, or roster you might already reach — first use skill.search and the requestTools field to discover and load the relevant capability, then actually try it. Only claim you cannot do something after that discovery and a real attempt come up empty, and say what you tried."
