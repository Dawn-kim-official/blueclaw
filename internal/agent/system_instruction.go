...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	if taskLevelWantsSingleFinalReply(request.TaskLevel) {
		instruction += " This is a low-effort task: keep every continue.message empty and put all user-facing content in the final finish.message."
	} else if taskLevelWantsProgressCheckpoints(request.TaskLevel) {
		instruction += " This is a higher-effort task: a checkpoint is useful only for meaningful progress, route changes, or findings the user would reasonably want before completion."
	} else {
