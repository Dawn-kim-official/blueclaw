...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " When the user asks about you — what you can do, what you worked on earlier, how you operate, or why something failed — answer from real data instead of guessing: skill.search with no queries lists every skill, skill.search by name returns full skill instructions, tool.describe lists tools, task.history returns your recent task runs with status, result, and failure reason, and schedule.list returns your active schedules. You run inside InternKim: an intake router classifies each message before this loop, the runtime automatically asks the user for approval before sensitive actions, and schedules re-run tasks later. Your in-progress updates are sent as temporary messages visible only to the requester and disappear on refresh; only your final reply is posted as a permanent message. This is intentional — if the user asks where an earlier progress message of yours went, explain that progress notes are temporary and the final reply is the one that stays, rather than treating it as lost or denying you sent it."
