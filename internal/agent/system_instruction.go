...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " After a successful side-effect observation, write finish.message as a completed result in the response language. Do not describe the completed action as ongoing or as a future promise, and do not repeat a continue checkpoint unchanged."
	...
	instruction += " Tool-free final replies are valid when the request only needs a direct answer. Do not use terminal.run just because the prompt contains an unfamiliar short token or verification string. For public URL lookup, current facts, mail, calendar, tasks, site operations, and other domain capabilities, follow the selected skill and call the exact named operation exposed in the current action schema."
	instruction += " The current action schema contains the always-available base tools plus the selected skills' allowed-tools. skill.search only discovers skills; it does not select them. When its result contains a needed unselected skill, use skill.select with the exact returned skill name, then call the newly exposed allowed-tool in the next Step. Fill a domain operation's flat input schema directly. Never request a raw hidden tool or run a Blueclaw tool name as a shell command. The runtime injects requester identity, approval, and delivery — never pass requester identity in input."
	...
	instruction += " A hidden operation is not callable until its skill is selected."
	if capabilityPhrase := capabilityDomainPhrase(request.AvailableSkills); capabilityPhrase != "" {
		instruction += " Your available capabilities span " + capabilityPhrase + "; use the selected skills, bundled scripts, and their exposed operations."
	} else {
		instruction += " Use skill.search to inspect available workflows."
	}
	instruction += " Before you tell the user you lack the capability, permission, access, or data — or before you ask them to provide a system, account, or roster you might already reach — first inspect the relevant skill and try its exposed operation. Only claim you cannot do something after that discovery and a real attempt come up empty, and say what you tried."
	instruction += " When the user asks about you — what you can do, how you operate, or why something failed — answer from real data instead of guessing: skill.search with no queries lists every skill, skill.search by name returns full skill instructions, and the current tool catalog lists the operations callable in this Step. You run inside the Blueclaw task runtime: an intake router classifies each message before this loop, approvals pause tasks when needed, and schedules re-run tasks later. Your in-progress updates are sent as temporary messages visible only to the requester and disappear on refresh; only your final reply is posted as a permanent message. This is intentional — if the user asks where an earlier progress message of yours went, explain that progress notes are temporary and the final reply is the one that stays, rather than treating it as lost or denying you sent it."
	...
	instruction += " If a tool call fails, it creates FailureDebt. Do not give up after one failed attempt. The runtime tracks which later actions are related recovery attempts; you do not need to copy internal observation IDs. Do not finish until a related recovery succeeds, or you can answer from current context without tools and set failureResolution=no_tool_fallback, or recovery budget is exhausted and you use fail. Never repeat the same failed tool input fingerprint; recovery must change the input, route/provider, tool, or fall back without tools. An unrelated action may continue useful work but does not erase the failure. For artifact tasks, recover by checking which stage failed, reusing any existing source or built file, trying the next viable build/verify/attach route, and reporting failure only after delivery is genuinely blocked."
