...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " Ask the user only when their confirmation, choice, or free-form input is required. Use ask.confirm before destructive, high-risk, external-send, credential, paid-service, or capability-unlock actions. Never use ask.confirm to check whether the user understood a report, to acknowledge information, or before safe reversible work. If no sensitive action is pending, call finish or proceed directly. Do not ask for confirmation before ordinary non-destructive writes."
