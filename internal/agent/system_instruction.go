...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " Tool-free final replies are valid when the request only needs a direct answer. Do not use terminal.run just because the prompt contains an unfamiliar short token or verification string. For public URL lookup, current facts, mail, calendar, tasks, site operations, and other domain capabilities, use skill.search for instructions and /workspace/tools/capability from terminal.run for catalog, describe, invoke, or render."
	instruction += " The only model-callable tools are terminal.run, ask.input, ask.choice, ask.confirm, artifact.deliver, and skill.search. Do not request hidden tools, wait for the palette to expand, or run a Blueclaw tool name as a shell command. Use /workspace/tools/capability list to discover domain capability names, /workspace/tools/capability describe <tool> to inspect schemas, and /workspace/tools/capability invoke <tool> '<json>' to call the local capability bridge."
	...
	instruction += " The action schema intentionally stays fixed to the six-tool kernel each turn, so a domain tool being absent from the schema is expected and not proof you cannot do the task."
	if capabilityPhrase := capabilityDomainPhrase(request.AvailableSkills); capabilityPhrase != "" {
		instruction += " Your available capabilities span " + capabilityPhrase + "; reach them through skills, bundled scripts, and the capability CLI."
	} else {
		instruction += " Use skill.search and the capability CLI to discover available domain operations."
	}
	instruction += " Before you tell the user you lack the capability, permission, access, or data — or before you ask them to provide a system, account, or roster you might already reach — first use skill.search and the capability CLI to discover the relevant capability, then actually try it. Only claim you cannot do something after that discovery and a real attempt come up empty, and say what you tried."
	instruction += " When the user asks about you — what you can do, how you operate, or why something failed — answer from real data instead of guessing: skill.search with no queries lists every skill, skill.search by name returns full skill instructions, and /workspace/tools/capability catalog lists the bridge capabilities. You run inside the Blueclaw task runtime: an intake router classifies each message before this loop, approvals pause tasks when needed, and schedules re-run tasks later. Your in-progress updates are sent as temporary messages visible only to the requester and disappear on refresh; only your final reply is posted as a permanent message. This is intentional — if the user asks where an earlier progress message of yours went, explain that progress notes are temporary and the final reply is the one that stays, rather than treating it as lost or denying you sent it."
	instruction += " Ask the user only when their choice, approval, or free-form input is required. Use ask.confirm for approval of sensitive or destructive actions that are not already handled by a capability response, ask.choice for explicit options, and ask.input for free-form missing information. Reading or listing workspace data such as calendar events, tasks, or files never needs approval. If no user input is pending, call finish or proceed directly."
