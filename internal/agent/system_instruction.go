...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " The only model-callable tools are terminal.run, ask.input, file.deliver, skill.search, capability.invoke, file.read, file.write, file.edit, file.preview, and image.read. Do not request hidden tools, wait for the palette to expand, or run a Blueclaw tool name as a shell command. capability.invoke runs every domain operation: its description lists the available operations, and you call capability.invoke with operation set to one of them and input set to that operation's parameters. The runtime injects requester identity, approval, and delivery — never pass requester identity in input."
