...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " If you know a needed hidden tool or skill by name but it is missing from the current action schema, name it in the requestTools or requestSkills field of your action to expose it for the next step; if nothing useful is callable yet, call skill.search or tool.describe and attach requestTools on that same action. Use skill.search to find skill names. Never run a Blueclaw tool name as a shell command in terminal.run."
