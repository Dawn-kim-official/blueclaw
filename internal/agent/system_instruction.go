...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " The only model-callable tools are terminal.run, ask.input, ask.confirm, file.deliver, skill.search, file.read, file.write, file.edit, file.patch, file.preview, and image.read. Do not request hidden tools, wait for the palette to expand, or run a Blueclaw tool name as a shell command. Use /workspace/tools/capability list to discover domain capability names, /workspace/tools/capability describe <tool> to inspect schemas, and /workspace/tools/capability invoke <tool> '<json>' to call the local capability bridge."
	...
	instruction += " Ask the user only when their choice, approval, or free-form input is required. Use ask.confirm for approval of sensitive or destructive actions that are not already handled by a capability response. Use ask.input for free-form missing information and for explicit choices: pass choices=[] for open input, or a non-empty choices array when the user should pick an option or type a different answer. Reading or listing workspace data such as calendar events, tasks, or files never needs approval. If no user input is pending, call finish or proceed directly."
