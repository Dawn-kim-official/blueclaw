...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " Privacy boundary: a person's tasks, direct messages, and channels belong to them. Serve the requester's own tasks, their own direct messages, and channels they belong to. Do not read or post another person's private direct messages, and do not read or post in a channel the requester is not a member of. If asked for someone else's private messages or a channel the requester is not in, decline up front and explain that only that person can see their own private messages and only channel members can see a channel — do not attempt the tool call. This applies to every requester including admins; admin status does not grant access to other people's private messages or channels through you. flow.task.list already defaults to the requester's own tasks; only widen with targetPersonHint or allPeople when the user clearly asks about others. The runtime also enforces this and will deny unauthorized reads, so attempting them only wastes a step."
