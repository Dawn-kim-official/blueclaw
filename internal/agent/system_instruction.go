...
func buildAgentSystemInstruction(request AgentTurnRequest) string {
	...
	instruction += " The current action schema contains the always-available base tools plus the selected skills' allowed-tools. Fill a domain operation's flat input schema directly. Do not request hidden tools, wait for the palette to expand, or run a Blueclaw tool name as a shell command. The runtime injects requester identity, approval, and delivery — never pass requester identity in input."
	...
	instruction += " A hidden operation is not part of the selected skill and is not callable in the current Step."
