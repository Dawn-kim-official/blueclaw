...
func TestBuildAgentActionRequestPreservesNativeToolCallingWireShape(t *testing.T) {
	...
	if !strings.Contains(request.StructuredOutputSchema.Document, `"action":{"enum":["request_tools"]`) {
		t.Fatalf("expected request_tools action in schema, got %s", request.StructuredOutputSchema.Document)
	}
...
func TestBuildAgentActionRequestIncludesApprovalUserFacingContract(t *testing.T) {
	...
	if !messagesContain(request.Messages, "테스트이한테 DM 보내줘") {
