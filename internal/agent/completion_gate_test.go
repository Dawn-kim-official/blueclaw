...
func TestAgentTurnRunnerAutoAttachmentSatisfiesPendingOperation(t *testing.T) {
	workspaceRootPath := t.TempDir()
	...
	turnStartedAt := time.Now().Add(-time.Minute)
	writeAgentTestFile(t, artifactPath, `{"status":"ready"}`)

	languageModel := &sequenceLanguageModel{contents: []string{
		finishMessageWithEvidence("report.json 파일을 첨부했습니다.", "obs-001", "file.deliver", 0),
	}}
	services := newTurnRunnerTestServices(languageModel, TurnOptions{})
	toolRegistry := newTestToolSet([]string{"file.deliver"})
	deliveryCount := 0
	registerTestTool(toolRegistry, ToolDefinition{Name: "file.deliver"}, func(_ context.Context, invocation ToolInvocation) (ToolResult, error) {
		...
		if input.Filename != "report.json" || len(input.Files) != 1 {
			return ToolResult{}, fmt.Errorf("unexpected delivery input %s", invocation.Input)
		}
		file := input.Files[0]
		if file.Path != artifactPath || file.Filename != "report.json" || file.Title != "분기 보고" {
			return ToolResult{}, fmt.Errorf("unexpected delivery input %s", invocation.Input)
		}
		return ToolResult{
			Output: ToolOutput{Content: "file attached"},
	...
	requiredInput := MarshalToolInput(map[string]any{
	...
	if errorValue != nil {
		t.Fatalf("expected typed auto delivery to succeed: %v", errorValue)
	}
	if result.TaskRun.Status != task.TaskStatusCompleted || deliveryCount != 1 {
		t.Fatalf("expected one completed delivery, got status=%s count=%d", result.TaskRun.Status, deliveryCount)
	}
	taskEvents := services.taskEventService.ListTaskEvent(result.TaskRun.TaskRunID)
	if taskEventsContain(taskEvents, "agent.operation_contract_input_rejected", "") {
		t.Fatalf("expected automatic delivery to satisfy pending operation: %+v", taskEvents)
	}
}
