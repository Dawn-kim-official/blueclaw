...
func TestAgentTurnRunnerAllowsCorrectedRetryAfterSafeFailure(t *testing.T) {
	languageModel := &sequenceLanguageModel{contents: []string{
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"샘플","message":"확인 부탁해"}}`,
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"이샘플","message":"확인 부탁해"}}`,
...
func TestAgentTurnRunnerRejectsSecondDMSendAfterSuccess(t *testing.T) {
	languageModel := &sequenceLanguageModel{contents: []string{
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"샘플","message":"첫 번째"}}`,
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"샘플","message":"두 번째"}}`,
...
func TestAgentTurnRunnerRejectsRepeatedFailedFingerprint(t *testing.T) {
	languageModel := &sequenceLanguageModel{contents: []string{
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"샘플","message":"확인 부탁해"}}`,
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"샘플","message":"확인 부탁해"}}`,
		`{"action":"continue","toolName":"platform.dm.inspect","toolInput":{"recipientHint":"샘플"}}`,
		`{"action":"continue","toolName":"platform.dm.inspect","toolInput":{"recipientHint":"이샘플"}}`,
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"정국","message":"확인 부탁해"}}`,
...
func TestAgentTurnRunnerRejectsUnsafeRepeatedExternalSend(t *testing.T) {
	languageModel := &sequenceLanguageModel{contents: []string{
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"샘플","message":"확인 부탁해"}}`,
		`{"action":"continue","toolName":"platform.dm.send","toolInput":{"recipientHint":"샘플","message":"확인 부탁해"}}`,
		failureReportDocument("send failed", "platform.dm.send", "샘플", FailureCodes.OperationFailed.String(), "message_send", "Mattermost returned 503 after post create"),
...
func TestAgentTurnRunnerExpectedResultVerifierBlocksEarlyFinish(t *testing.T) {
	languageModel := &sequenceLanguageModel{
		...
		contents: []string{
			...
			`{"action":"finish","message":"초안을 만들었습니다.","replyParts":[{"type":"text","text":"초안을 만들었습니다."}],"goalStatus":"satisfied","goalSatisfied":true,"completionEvidence":[{"observationID":"obs-001","toolName":"site.app.create"}]}`,
			...
			`{"action":"finish","message":"배포했습니다: https://portfolio.example","replyParts":[{"type":"text","text":"배포했습니다: https://portfolio.example"}],"goalStatus":"satisfied","goalSatisfied":true,"completionEvidence":[{"observationID":"obs-003","toolName":"site.app.publish"}]}`,
		},
	}
	services := newTurnRunnerTestServices(languageModel, TurnOptions{MaxIterationCount: 6})
...
func TestAgentTurnRunnerExpectedResultsDoNotRequireLegacyToolEvidenceFirst(t *testing.T) {
	languageModel := &sequenceLanguageModel{
		...
		contents: []string{
			...
			`{"action":"finish","message":"배포했습니다: https://portfolio.example","replyParts":[{"type":"text","text":"배포했습니다: https://portfolio.example"}],"goalStatus":"satisfied","goalSatisfied":true,"completionEvidence":[{"observationID":"obs-001","toolName":"site.app.publish"}]}`,
		},
	}
	services := newTurnRunnerTestServices(languageModel, TurnOptions{MaxIterationCount: 4})
...
func TestAgentTurnRunnerDoesNotApplySiteApprovalRejectToDirectMessage(t *testing.T) {
	languageModel := &sequenceLanguageModel{contents: []string{
		`{"action":"continue","toolName":"ask.confirm","toolInput":{"userFacingMessage":"샘플 님에게 DM을 보내도 될까요?","reasonCode":"external_send","reasonDetail":"external send"}}`,
...
func TestAgentTurnRunnerFinalizesScheduleCreateAfterSuccess(t *testing.T) {
	languageModel := &sequenceLanguageModel{contents: []string{
		`{"action":"continue","toolName":"schedule.create","toolInput":{"prompt":"죄송합니다.","kind":"interval","intervalSecond":60,"maxRunCount":10,"repeatPolicy":"finite","timeZone":"Asia/Seoul"}}`,
		`{"action":"continue","toolName":"schedule.create","toolInput":{"prompt":"죄송합니다.","kind":"interval","intervalSecond":60,"maxRunCount":10,"repeatPolicy":"finite","timeZone":"Asia/Seoul"}}`,
	}}
	services := newTurnRunnerTestServices(languageModel, TurnOptions{MaxIterationCount: 4, MaxToolCallCount: 4})
	toolCallCount := 0
...
func TestAgentTurnRunnerRejectsRepeatedScheduleCreateWithoutExecutingAgain(t *testing.T) {
	languageModel := &sequenceLanguageModel{contents: []string{
		`{"action":"continue","toolName":"schedule.create","toolInput":{"prompt":"죄송합니다.","kind":"interval","intervalSecond":60,"maxRunCount":10,"repeatPolicy":"finite","timeZone":"Asia/Seoul"}}`,
		`{"action":"continue","toolName":"schedule.create","toolInput":{"timeZone":"Asia/Seoul","maxRunCount":10,"repeatPolicy":"finite","intervalSecond":60,"kind":"interval","prompt":"죄송합니다."}}`,
