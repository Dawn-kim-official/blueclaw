...
func TestTaskIntakePlannerMapsModelRequiredEvidenceToInitialTools(t *testing.T) {
	languageModel := &sequenceLanguageModel{contents: []string{
	...
	for _, toolName := range []string{"task.add"} {
		if !containsString(decision.InitialToolNames, toolName) {
			t.Fatalf("expected initial evidence tool %s, got %+v", toolName, decision.InitialToolNames)
		}
	}
}
