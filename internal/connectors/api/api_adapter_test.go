...
func TestResolveIdentityResolvesApprovedEmail(t *testing.T) {
	adapter := NewAdapter(stubIdentityResolver{
		personIDByEmail: map[string]string{"lee@dawn.kim": "person-1"},
	...
	if identity.PersonID != "person-1" || identity.Platform != "api" || identity.Email != "lee@dawn.kim" {
...
func TestParseHTTPEventReadsNormalizedEvent(t *testing.T) {
	adapter := NewAdapter(stubIdentityResolver{}, NewReplyStore())
	body := `{"conversationID":"dm:api:p1","messageID":"m1","senderID":"lee@dawn.kim","replyTargetID":"dm:api:p1","prompt":"hi"}`
