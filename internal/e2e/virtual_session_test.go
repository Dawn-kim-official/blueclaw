...
func TestVirtualTaskCapabilityPreservesLifecycleState(t *testing.T) {
	...
	addResponse := service.response("task.add", []byte(`{"input":{"title":"비용 테스트 회귀 확인","goal":"회귀 방지","targetPersonHint":"세은","participantPersonHints":["동하"]},"context":{}}`))
	...
	if addDocument.Result["taskID"] != "task-1" ||
		...
		addDocument.Result["ownerName"] != "세은" {
	...
	if participantNames := stringSliceValue(addDocument.Result["participantNames"]); !slices.Equal(participantNames, []string{"동하"}) {
...
func TestVirtualSiteServeRequiresValidSourceBundle(t *testing.T) {
	...
	if !strings.Contains(response, `"status":"published"`) ||
		...
		!strings.Contains(response, `"url":"https://local-fleet-studio.device.intern.kim"`) {
	...
	if !strings.Contains(previewResponse, `"status":"previewed"`) ||
		!strings.Contains(previewResponse, `"previewURL":"https://local-fleet-studio.device.intern.kim/__preview/preview-1"`) ||
