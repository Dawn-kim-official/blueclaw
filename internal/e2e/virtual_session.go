...
func (service *virtualCapabilityService) response(toolName string, requestBody []byte) string {
	...
	case "site.serve":
		...
		if mode == "publish" {
			...
			result["publishedURL"] = "https://" + slug + ".device.intern.kim"
		} else {
			result["previewURL"] = "https://" + slug + ".device.intern.kim/__preview/preview-1"
		}
	...
	case "site.list":
		...
		if service.site != nil {
			...
			if service.sitePublished {
				...
				entry["publishedURL"] = "https://" + stringValue(service.site.Values["slug"]) + ".device.intern.kim"
			}
