		...
		Turns: []VirtualTurn{
			...
			},
			{
				...
				ExpectedEventCounts: []VirtualEventCount{
					...
					{Name: "approval.pending_call", BodyFragment: `"calendar.delete"`, Count: 1},
				},
			...
			},
			{
				...
				ActionResponses: []string{
					actionFinishMessage("제품 회고 일정을 삭제했습니다.", "obs-002:calendar.delete:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedEventCounts: []VirtualEventCount{
					{Name: "approval.executed", BodyFragment: `"calendar.delete"`, Count: 1},
				},
...
func AmbientDutyCalendarAcceptanceScenario(artifactDirectoryPath string) VirtualSessionScenario {
	return VirtualSessionScenario{
		...
		Turns: []VirtualTurn{{
			...
			ActionResponses: []string{
				actionInvokeCapabilityTool("calendar.add", `{"title":"정기회의","startISO":"2026-06-12T17:00:00+09:00","endISO":"2026-06-12T18:00:00+09:00","timeZone":"Asia/Seoul","people":["이찬희","이동하"]}`),
			...
			ExpectedEventCounts: []VirtualEventCount{
				...
				{Name: "tool.calendar.add.requested", BodyFragment: "이찬희", Count: 1},
				{Name: "tool.calendar.add.requested", BodyFragment: "이동하", Count: 1},
			},
			ExpectedModelContexts: []string{
...
func AmbientTaskCaptureAcceptanceScenario(artifactDirectoryPath string) VirtualSessionScenario {
	return VirtualSessionScenario{
		...
		Turns: []VirtualTurn{{
			...
			Addressing:             connectors.AddressingMetadata{OtherPersonMentioned: true},
			ActionResponses: []string{
				actionInvokeCapabilityTool("task.update", `{"taskHint":"task-1","endDate":"2026-06-24"}`),
				actionFinishMessage("세은 님 업무 마감을 수요일로 변경했습니다.", "obs-001:task.update:0"),
			},
			CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
	...
	return VirtualSessionScenario{
		...
		Turns: []VirtualTurn{{
			...
			ActionResponses: []string{
				...
				actionInvokeCapabilityTool("task.update", `{"taskHint":"task-1","endDate":"2026-07-24"}`),
...
func SitePrototypeAcceptanceScenario(artifactDirectoryPath string) VirtualSessionScenario {
	return VirtualSessionScenario{
		...
		Turns: []VirtualTurn{{
			...
			ActionResponses: []string{
				actionCallTool("file.write", `{"path":"/workspace/circles/staff/sites/local-fleet-studio/draft/app/public/site-content.json","content":"{\"siteName\":\"Local Fleet Studio\",\"tagline\":\"로컬 플릿 웹사이트 생성 배포 테스트\",\"blocks\":[{\"variant\":\"hero\",\"title\":\"Local Fleet Studio\",\"body\":\"로컬 플릿 웹사이트 생성 배포 테스트\"},{\"variant\":\"prose\",\"title\":\"서비스 소개\",\"body\":\"Local Fleet Studio는 로컬 플릿 환경에서 웹사이트 생성과 배포 과정을 검증하는 테스트 서비스입니다.\"},{\"variant\":\"features\",\"title\":\"장점\",\"items\":[{\"title\":\"빠른 프로토타입\",\"body\":\"빠른 프로토타입 생성\"},{\"title\":\"안전한 검증\",\"body\":\"안전한 배포 검증\"},{\"title\":\"손쉬운 재배포\",\"body\":\"손쉬운 재배포\"}]},{\"variant\":\"cta\",\"title\":\"문의\",\"body\":\"자세한 내용이 궁금하시면 지금 바로 문의해 주세요.\"}]}"}`),
				actionInvokeCapabilityTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/local-fleet-studio/draft","mode":"publish"}`),
				actionFinishMessage("Local Fleet Studio 웹사이트 프로토타입을 배포했습니다: https://local-fleet-studio.device.intern.kim", "obs-002:site.serve:0"),
			},
			CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
			ExpectedSelectedSkills:   []string{"website"},
			ExpectedToolCallCounts:   map[string]int{"terminal.run": 0},
			ExpectedEventCounts: []VirtualEventCount{
				{Name: "tool.site.serve.requested", BodyFragment: "site.serve", Count: 1},
				{Name: "tool.site.serve.result", BodyFragment: "device.intern.kim", Count: 1},
			},
			ExpectedModelContexts:  []string{"site.serve", "Local Fleet Studio"},
			ForbiddenModelContexts: []string{"home/sites/site-1"},
			ExpectedReplyFragments: []string{"https://local-fleet-studio.device.intern.kim"},
			ForbiddenReplyFragments: []string{
				"죄송",
				"완료하지 못",
				"기능은 제공",
		...
		Turns: []VirtualTurn{
			{
				...
				ActionResponses: []string{
					...
					actionInvokeCapabilityTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/local-fleet-studio/draft","mode":"publish"}`),
					actionFinishMessage("Deployed the Local Fleet Studio site: https://local-fleet-studio.device.intern.kim", "obs-002:site.serve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedSelectedSkills:   []string{"website"},
				ExpectedToolCallCounts:   map[string]int{"terminal.run": 0},
			...
			},
			{
				...
				ActionResponses: []string{
					actionCallTool("site.list", `{}`),
					actionCallTool("file.write", `{"path":"/workspace/circles/staff/sites/local-fleet-studio/draft/app/public/site-content.json","content":"{\"siteName\":\"Local Fleet Studio Updated\",\"tagline\":\"Redeploy verification passed\",\"blocks\":[{\"variant\":\"hero\",\"title\":\"Local Fleet Studio Updated\",\"body\":\"Redeploy verification passed\"}]}"}`),
					actionInvokeCapabilityTool("site.serve", `{"title":"Local Fleet Studio Updated","sourceWorkspacePath":"/workspace/circles/staff/sites/local-fleet-studio/draft","mode":"publish","siteReference":"local-fleet-studio"}`),
					actionFinishMessage("Updated and redeployed the site: https://local-fleet-studio.device.intern.kim", "obs-002:file.write:0", "obs-003:site.serve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedToolCallCounts:   map[string]int{"terminal.run": 0},
				ExpectedEventCounts: []VirtualEventCount{
					{Name: "tool.site.list.requested", BodyFragment: "site.list", Count: 1},
	...
	return VirtualSessionScenario{
		...
		Turns: []VirtualTurn{{
			...
			ActionResponses: []string{
				...
				actionCallTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/demo/draft","mode":"publish","siteReference":"demo"}`),
				...
				actionCallTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/demo/draft","mode":"publish","siteReference":"demo"}`),
				actionFinishMessage("커스텀 레이아웃을 빌드하고 다시 배포했습니다: https://demo.device.intern.kim", "obs-004:site.serve:0"),
			},
			CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
			ExpectedToolCallCounts:   map[string]int{"terminal.run": 1, "file.write": 1, "site.serve": 1},
			ExpectedEventCounts: []VirtualEventCount{
			...
			ExpectedModelContexts:  []string{"site.serve requires a fresh build", "app/src", "app/public/site-content.json"},
			ForbiddenModelContexts: []string{"home/sites/site-1"},
		...
		Turns: []VirtualTurn{
			{
				...
				ActionResponses: []string{
					actionCallTool("terminal.run", `{"command":"mkdir -p dist && printf '<!doctype html><html><body><main><h1>Local Fleet Studio</h1><p>로컬 플릿 웹사이트 CRUD 테스트</p></main></body></html>' > dist/index.html","workingDirectoryPath":"/workspace/circles/staff/sites/local-fleet-studio/draft/app","timeoutSecond":120}`),
					actionInvokeCapabilityTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/local-fleet-studio/draft","mode":"publish"}`),
					actionFinishMessage("Local Fleet Studio 웹사이트를 배포했습니다: https://local-fleet-studio.device.intern.kim", "obs-002:site.serve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedSelectedSkills:   []string{"website"},
				ExpectedEventCounts: []VirtualEventCount{
			...
			},
			{
				...
				ActionResponses: []string{
					actionCallTool("site.list", `{}`),
					actionCallTool("file.write", `{"path":"/workspace/circles/staff/sites/local-fleet-studio/draft/app/src/App.tsx","content":"export default function App() {\n  return <main><h1>Local Fleet Studio Updated</h1><p>재배포 검증 완료</p></main>;\n}\n"}`),
					actionCallTool("terminal.run", `{"command":"mkdir -p dist && printf '<!doctype html><html><body><main><h1>Local Fleet Studio Updated</h1><p>재배포 검증 완료</p></main></body></html>' > dist/index.html","workingDirectoryPath":"/workspace/circles/staff/sites/local-fleet-studio/draft/app","timeoutSecond":120}`),
					actionInvokeCapabilityTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/local-fleet-studio/draft","mode":"publish","siteReference":"local-fleet-studio"}`),
					actionFinishMessage("Local Fleet Studio 웹사이트를 수정하고 다시 배포했습니다: https://local-fleet-studio.device.intern.kim", "obs-002:file.write:0", "obs-004:site.serve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedEventCounts: []VirtualEventCount{
					{Name: "tool.site.list.requested", BodyFragment: "site.list", Count: 1},
			...
			},
			{
				...
				ActionResponses: []string{
					actionCallTool("site.list", `{}`),
					actionCallToolWithMessage("site.unserve", "Local Fleet Studio 테스트 웹사이트를 삭제합니다.", `{"siteReference":"local-fleet-studio"}`),
				},
				ExpectedEventCounts: []VirtualEventCount{
					{Name: "tool.site.list.requested", BodyFragment: "site.list", Count: 1},
					{Name: "tool.site.unserve.requested", BodyFragment: "site.unserve", Count: 0},
					{Name: "tool.site.unserve.result", BodyFragment: "approval_required", Count: 0},
					{Name: "approval.pending_call", BodyFragment: `"site.unserve"`, Count: 1},
			...
			},
			{
				...
				RouterApproval: "approve",
				ActionResponses: []string{
					actionFinishMessage("Local Fleet Studio 테스트 웹사이트를 삭제했습니다.", "obs-002:site.unserve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedEventCounts: []VirtualEventCount{
					{Name: "tool.site.unserve.requested", BodyFragment: "site.unserve", Count: 1},
					{Name: "tool.site.unserve.result", BodyFragment: "unserved", Count: 1},
					{Name: "approval.executed", BodyFragment: `"site.unserve"`, Count: 1},
				},
				ExpectedEvents:         []string{"confirmation.reply_classified"},
				ExpectedReplyFragments: []string{"삭제했습니다"},
			},
