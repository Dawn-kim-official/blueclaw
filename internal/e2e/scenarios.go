	...
	return VirtualSessionScenario{
		...
		Turns: []VirtualTurn{{
			...
			ExpectedEventCounts: []VirtualEventCount{
				...
				{Name: "tool.task.add.result", BodyFragment: `"ownerName":"예시"`, Count: 1},
	...
	return VirtualSessionScenario{
		...
		Turns: []VirtualTurn{{
			...
			ActionResponses: []string{
				...
				actionFinishMessage("Local Fleet Studio 웹사이트 프로토타입을 배포했습니다: https://local-fleet-studio.device.example.test", "obs-002:site.serve:0"),
			},
			CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
			...
			ExpectedEventCounts: []VirtualEventCount{
				...
				{Name: "tool.site.serve.result", BodyFragment: "device.example.test", Count: 1},
			},
		...
		Turns: []VirtualTurn{
			{
				...
				ActionResponses: []string{
					...
					actionInvokeCapabilityTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/local-fleet-studio/draft","mode":"publish"}`),
					actionFinishMessage("Deployed the Local Fleet Studio site: https://local-fleet-studio.device.example.test", "obs-002:site.serve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedSelectedSkills:   []string{"website"},
				ExpectedToolCallCounts:   map[string]int{"terminal.run": 0},
			...
			},
			{
				...
				ActionResponses: []string{
					...
					actionFinishMessage("Updated and redeployed the site: https://local-fleet-studio.device.example.test", "obs-002:file.write:0", "obs-003:site.serve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedToolCallCounts:   map[string]int{"terminal.run": 0},
				ExpectedEventCounts: []VirtualEventCount{
	...
	return VirtualSessionScenario{
		...
		Turns: []VirtualTurn{{
			...
			ActionResponses: []string{
				...
				actionCallTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/demo/draft","mode":"publish","siteReference":"demo"}`),
				actionFinishMessage("커스텀 레이아웃을 빌드하고 다시 배포했습니다: https://demo.device.example.test", "obs-004:site.serve:0"),
			},
			CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
		...
		Turns: []VirtualTurn{
			{
				...
				ActionResponses: []string{
					...
					actionInvokeCapabilityTool("site.serve", `{"title":"Local Fleet Studio","sourceWorkspacePath":"/workspace/circles/staff/sites/local-fleet-studio/draft","mode":"publish"}`),
					actionFinishMessage("Local Fleet Studio 웹사이트를 배포했습니다: https://local-fleet-studio.device.example.test", "obs-002:site.serve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedSelectedSkills:   []string{"website"},
				ExpectedEventCounts: []VirtualEventCount{
			...
			},
			{
				...
				ActionResponses: []string{
					...
					actionFinishMessage("Local Fleet Studio 웹사이트를 수정하고 다시 배포했습니다: https://local-fleet-studio.device.example.test", "obs-002:file.write:0", "obs-004:site.serve:0"),
				},
				CompletionJudgeResponses: []string{completionJudgeSatisfiedResponse()},
				ExpectedEventCounts: []VirtualEventCount{
					{Name: "tool.site.list.requested", BodyFragment: "site.list", Count: 1},
