...
func TestSimpleSlidesAssetsStayPureSkill(t *testing.T) {
	...
	if !strings.Contains(skillDocument, "NAME=<deck-slug> ./build.sh") {
		t.Fatal("SKILL.md should guide agents to build with build.sh")
	}
	if !strings.Contains(skillDocument, "FORMATS=html") {
		t.Fatal("SKILL.md should explain html-only builds")
	}

	buildScript := readTestFile(t, filepath.Join(skillDirectoryPath, "assets", "build.sh"))
