...
func TestSkillSearchToolExactNameIncludesCompletionMetadata(t *testing.T) {
	...
	toolCatalogBuilder.UseSkillSearch(skillSearchTestRetriever{}, func() agent.InstructionBundle {
		return agent.InstructionBundle{Skills: []agent.SkillInstruction{{
			...
			Source: agent.InstructionSource{Path: "skills/website/SKILL.md"},
		}}}
	})
	toolRegistry := toolCatalogBuilder.BuildToolSet(ToolCatalogRequest{ProfileName: "default"})
	...
	result, errorValue := toolRegistry.Invoke(context.Background(), agent.ToolInvocation{
		ToolName: "skill.search",
		Input: agent.MarshalToolInput(map[string]any{
			"queries": []map[string]string{{"description": "website"}},
			"limit":   5,
		}),
	})
	if errorValue != nil {
	...
	if !strings.Contains(result.ContentText(), `"sourcePath":"skills/website/SKILL.md"`) || !strings.Contains(result.ContentText(), "site.publish") {
...
func TestSkillManagementRejectsInvalidAndBuiltInNames(t *testing.T) {
	...
	for _, input := range []map[string]string{
		...
		{"name": "website", "content": userSkillDocument("website")},
		{"name": "document-analysis", "content": userSkillDocument("document-analysis")},
		{"name": "website-management", "content": userSkillDocument("website-management")},
	} {
		result, errorValue := toolRegistry.Invoke(context.Background(), agent.ToolInvocation{
			ToolName: "skill.add",
...
func TestSkillSearchToolListsAllSkillsWithoutQueries(t *testing.T) {
	...
	toolCatalogBuilder.UseSkillSearch(skillSearchTestRetriever{}, func() agent.InstructionBundle {
		return agent.InstructionBundle{Skills: []agent.SkillInstruction{
			...
			{Name: "website", Description: "Create sites.", AllowedTools: []string{"site.create"}},
		}}
	})
	toolRegistry := toolCatalogBuilder.BuildToolSet(ToolCatalogRequest{ProfileName: "default"})
	...
	if len(resultDocument.Skills) != 2 || resultDocument.Skills[0].Name != "mail" || resultDocument.Skills[1].Name != "website" {
...
func TestSkillSearchToolNameLookupReturnsPromptBody(t *testing.T) {
	...
	result, errorValue := toolRegistry.Invoke(context.Background(), agent.ToolInvocation{
		ToolName: "skill.search",
		Input:    agent.MarshalToolInput(map[string]any{"name": "WEBSITE"}),
	})
	if errorValue != nil {
		t.Fatal(errorValue)
	}
	...
	if resultDocument.Skills[0].SourcePath != "skills/website/SKILL.md" {
		t.Fatalf("expected source path, got %+v", resultDocument.Skills[0])
	}
}

func TestSkillSearchToolNameLookupHiddenSkillReturnsNotFound(t *testing.T) {
	toolCatalogBuilder := NewToolCatalogBuilder()
	toolCatalogBuilder.UseSkillSearch(skillSearchTestRetriever{}, func() agent.InstructionBundle {
		return agent.InstructionBundle{Skills: []agent.SkillInstruction{{
			Name:        "visible-skill",
	...
	result, errorValue := toolRegistry.Invoke(context.Background(), agent.ToolInvocation{
		ToolName: "skill.search",
		Input:    agent.MarshalToolInput(map[string]any{"name": "hidden-skill"}),
	})
	if errorValue != nil {
		t.Fatal(errorValue)
	}
	if !result.Failed() || result.FailureCode() != agent.FailureCodes.NotFound.String() {
		t.Fatalf("expected hidden skill to be not found, got %+v", result)
	}
}

func TestSkillSearchToolHidesCircleHiddenSkillsFromList(t *testing.T) {
	toolCatalogBuilder := NewToolCatalogBuilder()
	toolCatalogBuilder.UseSkillSearch(skillSearchTestRetriever{}, func() agent.InstructionBundle {
		return agent.InstructionBundle{Skills: []agent.SkillInstruction{
			{Name: "mail", Description: "Email skill."},
			{Name: "hr-review", Description: "HR review skill.", Prompt: "Confidential flow.", HiddenFromCircles: []string{"staff"}},
		}}
	})
	...
	if len(resultDocument.Skills) != 1 || resultDocument.Skills[0].Name != "mail" {
		t.Fatalf("expected circle-hidden skill to be excluded from list, got %+v", resultDocument.Skills)
	}
}
