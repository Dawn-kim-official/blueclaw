...
func TestApprovalRequestStoresUserFacingMessageSeparatelyFromReasonDetail(t *testing.T) {
	...
	toolResult, errorValue := toolRegistry.Invoke(agent.WithUserFacingMessage(agent.WithResponseLanguage(agent.WithTaskRunID(context.Background(), taskRun.TaskRunID), agent.ResponseLanguageKorean), "동하 님에게 다음 DM을 보내도 될까요?\n\n테스트"), agent.ToolInvocation{
		ToolName: "ask.confirm",
